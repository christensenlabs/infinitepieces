...
export default function SessionView({ programs, activeClient, sessionData, setSessionData }) {
  ...
  return (
    <div className="h-full flex flex-col bg-dfp rounded-[2.5rem] border border-dfp-border shadow-2xl overflow-hidden animate-in zoom-in-95">
       <header className="h-16 bg-dfp-dark/90 flex items-center justify-between px-6 border-b border-dfp-border shrink-0">
       ...
       <div className="flex flex-1 overflow-hidden">
          {/* Sidebar */}
          <div className="w-72 bg-dfp-light/50 border-r border-dfp-border overflow-y-auto custom-scrollbar p-4 space-y-2">
            ...
            {programs.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => setSelectedId(p.id)} className={`w-full text-left p-4 rounded-2xl transition-all border ${isSelected ? "bg-cyan-500/10 border-cyan-500/50" : "hover:bg-dfp-border/30 border-transparent"}`}>
          ...
          {/* Main Area */}
          <div className="flex-1 p-8 overflow-y-auto custom-scrollbar bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-dfp-light to-dfp">
            <div className="max-w-3xl mx-auto space-y-6">
              <div className="bg-dfp-light/80 rounded-[2rem] border border-dfp-border p-8 relative">
                ...
                <div className="bg-dfp p-4 rounded-xl border border-dfp-border text-sm text-slate-300 font-medium">
                  {activeP?.description}
                </div>
              </div>
              ...
              <div className="bg-dfp-light/80 rounded-[2.5rem] border border-dfp-border p-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  {['IND', 'V', 'M', 'ERR'].map((label) => (
                    <button key={label} onClick={() => recordTrial(label)} className={`h-24 rounded-[2rem] border-b-[6px] flex flex-col items-center justify-center transition-all active:translate-y-1 active:border-b-0 shadow-lg ${label === 'IND' ? 'bg-green-500 border-green-700 text-white' : label === 'ERR' ? 'bg-rose-500 border-rose-700 text-white' : 'bg-blue-500 border-blue-700 text-white'}`}>
                      <span className="font-black text-3xl">{label}</span>
                    </button>
                  ))}
                </div>

                <div className="bg-dfp rounded-2xl border border-dfp-border p-6 min-h-[100px] flex flex-wrap gap-2 items-start">
