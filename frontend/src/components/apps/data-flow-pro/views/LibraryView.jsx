    ...
    <div className="max-w-5xl mx-auto space-y-6 animate-in fade-in duration-500">
      ...
      <div className="space-y-4">
        {programs.map(p => {
          ...
          return (
            <div key={p.id} className={`bg-dfp-light/80 rounded-[2rem] border transition-all ${isOpen ? 'border-cyan-400 shadow-[0_0_20px_rgba(0,229,255,0.1)]' : 'border-dfp-border'}`}>
              <div onClick={() => setExpandedId(isOpen ? null : p.id)} className="p-6 cursor-pointer flex justify-between items-start hover:bg-dfp/50 rounded-[2rem]">
                <div>
              ...
              {isOpen && (
                <div className="p-6 bg-dfp border-t border-dfp-border rounded-b-[2rem] animate-in slide-in-from-top-2 text-sm text-slate-300">
