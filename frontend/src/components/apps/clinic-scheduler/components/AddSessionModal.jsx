      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden p-6 sm:p-8 zoom-in-95 flex flex-col max-h-[90vh]">
        ...
        <div className="overflow-y-auto pr-2 custom-scrollbar">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            <div><label className="block text-sm font-bold text-slate-700 mb-1.5">{newSession.category === 'admin' ? 'Task / Event Name' : 'Patient Name'}</label><input required type="text" value={newSession.patientName} onChange={e=>setNewSession({...newSession, patientName: e.target.value})} className={schedulerStyles.input}/></div>
            <div>
            ...
            <div><label className="block text-sm font-bold text-slate-700 mb-1.5">Date</label><input required type="date" value={newSession.date} onChange={e=>setNewSession({...newSession, date: e.target.value})} className={schedulerStyles.input}/></div>
            <div className="grid grid-cols-2 gap-4">
              <div><label className="block text-sm font-bold text-slate-700 mb-1.5">Start Time</label><input required type="time" value={newSession.startTime} onChange={e=>setNewSession({...newSession, startTime: e.target.value})} className={schedulerStyles.input}/></div>
              <div><label className="block text-sm font-bold text-slate-700 mb-1.5">End Time</label><input required type="time" value={newSession.endTime} onChange={e=>setNewSession({...newSession, endTime: e.target.value})} className={schedulerStyles.input}/></div>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {newSession.category !== 'admin' ? (
                <div><label className="block text-sm font-bold text-slate-700 mb-1.5">Service Type</label><select value={newSession.type} onChange={e=>setNewSession({...newSession, type: e.target.value})} className={schedulerStyles.select}>{(config?.sessionTypes?.clinical || ['1:1 Therapy', 'Group Therapy', 'Parent Training']).map(t => <option key={t}>{t}</option>)}</select></div>
              ) : (
                <div><label className="block text-sm font-bold text-slate-700 mb-1.5">Work Type</label><select value={newSession.type} onChange={e=>setNewSession({...newSession, type: e.target.value})} className={schedulerStyles.select}>{(config?.sessionTypes?.admin || ['Prep / Planning', 'Drive Time', 'Meeting', 'Other Non-Billable']).map(t => <option key={t}>{t}</option>)}</select></div>
              )}
              <div><label className="block text-sm font-bold text-slate-700 mb-1.5">Location</label><input required type="text" value={newSession.location} onChange={e=>setNewSession({...newSession, location: e.target.value})} className={schedulerStyles.input}/></div>
            </div>
